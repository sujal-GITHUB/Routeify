...
const Home = () => {
  ...
  if (pickup && destination) {
    ...
    .fromTo(vehiclePanelRef.current,
      { 
        opacity: 0, 
        height: 0,
        y: 20
...
useEffect(() => {
  ...
  if (panelOpen) {
    tl
      .set(panel, { display: "block", opacity: 0 })
      .to(panel, {
        height: "75%",
        opacity: 1,
  ...
  // Add this new function to handle panel toggle animation
const handleVehiclePanelToggle = () => {
  ...
  return (
    <div className="h-screen font-lexend relative">
      ...
      <div ref={mainPanel} className="absolute inset-0 flex flex-col justify-end">
                ...
                <i className="ri-logout-box-line text-xl" />
                 ...
                 {showRideOptions ? ( 
                  <button 
                    ...
                    className="opacity-1"
                  >
                    <i className={`ri-arrow-${up ? 'up' : 'down'}-s-line text-xl`} />
                  </button>
                 ) :
                 (
                  <button 
