      ...
      <div className="flex items-center gap-3 mb-3 w-full">
        <div className="flex flex-col w-full">
          ...
          {locations.map((item, index) => (
            <div
              key={index}
              onClick={() => submitDestination(item.location)}  
              className={`mb-3 bg-white w-full p-3 rounded-xl border-2 border-transparent hover:border-black active:border-black transition-all duration-300 ${destination === item.location ? 'border-black' : ''}`}
            >
